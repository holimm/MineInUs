...
$rownum = 0;
$query = $conn->query("SELECT * FROM `trans_log_other` WHERE name='$realname' ORDER BY `id` DESC LIMIT 10");
if($query->num_rows > 0){ 
    while($row = $query->fetch_array())
	{
		...
		<tbody>
			<tr>
			  ...
			  <td>
				<p class='fw-normal mb-1'>".strtoupper($row['type'])."-".$row['id']."</p>
			  </td>
			  <td>
			  ...
			  <td>
				<p class='fw-normal mb-1'>".$row['dateput']."</p>
			  </td>
			</tr>
		  </tbody>
		";
